...
author: Tyler
date: '2020-04-17'
slug: website-design-part-2
keywords:
  - site design
summary: "This post continues the work from the previous one.  This code takes the saved data and creates the images."
thumbnailImage: /post/2020-04-17-website-design-part-2_files/color_scheme_thumbnail.png
...
This post continues the work from the [previous one](/2020/04/16/website-design-part-1/).  This code takes the saved data and creates the images.  The first two sections set up the rest of the work.
...
:::
::: {}
```{r color_scheme_image, eval = TRUE, echo = FALSE}
knitr::include_graphics('/post/2020-04-17-website-design-part-2_files/color_scheme.png', error = FALSE)
```
:::
::::
...
```{r cover_image, eval = TRUE, echo = FALSE}
knitr::include_graphics('/post/2020-04-17-website-design-part-2_files/cover.png', error = FALSE)
```
...
```{r icon_image, eval = TRUE, echo = FALSE, out.extra='height = 200px; style="display:block; margin:auto;"'}
knitr::include_graphics('/post/2020-04-17-website-design-part-2_files/icon.png', error = FALSE)
```

The background is transparent so that it doesn’t appear out of place when moved.  The bars come from the same data as the background but only using the letters of my name.  The  config.toml setting is `favicon = "images/website/icon.png"`.  The source code file resides [here](https://github.com/WilliamTylerBradley/site_design/blob/master/make_images.R){target="_blank"}.